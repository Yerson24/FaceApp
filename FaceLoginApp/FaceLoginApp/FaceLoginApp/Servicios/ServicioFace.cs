    ...
    public static class ServicioFace
    {
        public static async Task<Emocion> ObtenerEmocion(MediaFile foto) 
        {
            ...
            try
            {
                if (foto != null)
                {
                    var clientFace = new FaceServiceClient(Constantes.FaceApiKey, Constantes.FaceApiURL);
                    var atributosFace = new FaceAttributeType[] { FaceAttributeType.Emotion };

                    using (var stream = foto.GetStream())
                    {
                        Face[] rostros = await clientFace.DetectAsync(stream, false, false, atributosFace);
        ...
        public static async Task<bool> CrearGrupoEmpleados()
        {
            try
            {
                ...
                await clienteFace.CreatePersonGroupAsync(Constantes.FaceGroupID, Constantes.FaceGroupName,
                    Constantes.FaceGroupDescription);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async static Task<Guid> ReigstrarPersonaEnGrupo(string nombre)
        {
            try
            {
                var clienteFace = new FaceServiceClient(Constantes.FaceApiKey, Constantes.FaceApiURL);
