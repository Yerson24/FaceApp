        ...
        async void btnTomarFoto_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (foto1 == null)
                {
                    ...
                    if (foto1 != null)
                        image1.Source = ImageSource.FromStream(foto1.GetStream);
                }
                else if (foto2 == null)
                {
                    foto2 = await ServicioImagen.TomarFoto();
                    if (foto2 != null)
                        image2.Source = ImageSource.FromStream(foto2.GetStream);
                }
                else
                {
                    foto3 = await ServicioImagen.TomarFoto();
                    if (foto3 != null)
                        image3.Source = ImageSource.FromStream(foto3.GetStream);
                }
            }
            catch (Exception ex)
            {
        ...
        async void btnRegistrar_Clicked(object sender, EventArgs e)
        {
            if (foto1 != null && foto2 != null && foto3 != null)
            {
                ...
                try
                {
                    ...
                    var personID = await ServicioFace.ReigstrarPersonaEnGrupo(nombre);
